...
ß
    public BotMove(CricketPresence p, BotGame pg, Response r){
        _bg = pg;
        if (p!=null){
            _p = p;
            if (_p.isShill()){
    ...
    public void executeMove(){
        ...
        if (r != null && r instanceof Response){
            _cat.finest("Response=" + r.getBroadcast() + " " + _bp);
            _bg.deliverResponse((Response)r);
        }
    }
    ...
    public _Move getBestMove(String moves[][]){
        int move_pos =  Integer.parseInt(moves[0][0]);
        CricketPresence move_p  = _bg._pg.getPlayerAtPos(move_pos);
