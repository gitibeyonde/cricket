...
import com.cricket.mmog.GameStateEvent;
...
public class BotGame extends TimerTask implements Observer {
    ...
    public BotGame(Cricket pg, int type){
        _pg = pg;
        _type = type;
    ...
    public synchronized void update(Observable o, Object arg) {
       ß if (arg == GameStateEvent.GAME_SETUP) {
            if (_keepRunning){
                Cricket pg = (Cricket) o;
                for (CricketPresence p: pg.allPlayers(0)){
        ...
        if (_type == RANDOM){
            ...
            for (int i=0;i<_pg.maxPlayers();i++){
                if (Math.random() > 0.5){
                    ...
                    CricketPresence p = bp.createPresence(_pg._name);// create presence
                    p.setPos(i);
                    PokerResponse r = (CricketResponse)_pg.join(p, bp._initialChips);
    ...
    private synchronized void  seatPlayer(BotGame bg, int pos){
        ...
        try {
            ...
            if (pg.type().isRandomBotGame()){
                for (int i=0;i<10;i++){
                    new_pos = com.poker.shills.BotData.randomInt(pg.maxPlayers());
    ...
    public synchronized void run() {
        try {
            ...
            while (bm != null){
                ...
                if (bm._pos == -1) {
                    Thread.sleep(com.poker.shills.BotData.randomInt(com.poker.shills.BotData.NET_DELAY + com.poker.shills.BotData.randomInt(com.poker.shills.BotData.MOVE_DELAY)));
                    bm.executeMove();
                }
                else {
